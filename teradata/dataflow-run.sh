...
gcloud dataflow flex-template run ${name} \
    --template-file-gcs-location gs://${storage_bucket}/dataflow/flex-template/loadtdtable.json \
    --region ${location} \
    --parameters table_name="FRUITS" \
    --parameters schema_name="PRD_ADS_FRUIT_GCP_DB" \
    --parameters prjid="${project_id}" \
